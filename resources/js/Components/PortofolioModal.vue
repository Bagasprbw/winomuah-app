...
// Update gambar utama saat prop berubah
watch(
    () => props.portofolio,
    (val) => {
...
// ====== Thumbnails ======
// Memastikan thumbnails selalu ada isinya (sesuai kodemu sebelumnya)
const thumbnails = ref([])
watch(
    () => props.portofolio,
    (val) => {
        if (!val) return
        // Dummy thumbnails dari gambar yang sama (sesuai request "jangan ubah tampilan")
        thumbnails.value = [val.image, val.image, val.image]
            .filter(Boolean)
            .map(resolveImage)
    },
    { immediate: true }
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2">
                    <div class="modal-left bg-white p-8 lg:p-12 flex flex-col justify-center">
                        <div class="w-full max-w-md rounded-2xl overflow-hidden bg-gray-100 shadow-xl mx-auto">
                            <img v-if="selectedImage" :src="selectedImage" :alt="portofolio.title"
                    ...
                    <div class="modal-right bg-[#FAF8F3] p-8 lg:p-12 flex flex-col justify-center">
                        <div class="space-y-8 max-w-lg">
                            <nav class="info-item flex items-center gap-2 text-sm">
                                <span
                                    class="text-amber-600 hover:text-amber-700 font-semibold cursor-pointer transition-colors">
                                    Portofolio
                                </span>
                            ...
                            <div class="info-item">
                                ...
                                <p class="text-gray-700 leading-relaxed">
                                    {{ portofolio.description || 'Deskripsi portofolio belum tersedia.' }}
                                </p>
                            </div>
                            ...
                            <div class="info-item">
                                ...
                                <div class="flex flex-wrap gap-2">
                                    <span
                                        ...
                                        {{ portofolio.category?.name ?? portofolio.category ?? 'Proyek Seni' }}
                                    </span>
                                </div>
                            </div>

                            <div class="info-item pt-2 flex justify-center">
                                <a :href="`https://wa.me/${usePage().props.company.phone}?text=Hi, saya tertarik dengan portofolio ${portofolio.title}`"
                                    target="_blank"
